<template>
  <!-- TO-DO: Add this functionality to a dialog -->
  <q-page class="bg-green-1 row justify-center items-center">
    ...
    >
      <q-card square bordered class="q-pa-sm shadow-1">
        <q-card-section>
          <p class="login col-12 text-h6 text-left">Add Category</p>
        </q-card-section>
        <q-separator inset />
        <q-card-section>
          <div class="col-md-4 col-sm-6 col-xs-10 q-gutter-y-md">
            <q-input
              ...
              hint="Enter the name of the new category"
            >
    ...
    const handlerSubmit = async () => {
      try {
        if (isUpdate.value) {
          ...
          notifySuccess('Updated category')
        } else {
          await post(table, form.value)
          notifySuccess('Registered Category')
        }
