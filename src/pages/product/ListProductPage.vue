  ...
  <q-page padding>
    <div class="row">
      <q-table
        ...
        row-key="id"
        class="col-12"
        no-data-label="Nothing was found ..."
        no-results-label="No results for this filter."
        :rows="products"
      ...
      >
        ...
        <template v-slot:body-cell-actions="props">
          <q-td :props="props" class="q-gutter-x-sm">
            ...
            >
              <q-tooltip> Edit category </q-tooltip></q-btn
            >
            <q-btn
...
export default defineComponent({
  ...
  setup () {
    ...
    const handlerRemove = async category => {
      dialogShow({
        message: `Do you want to remove the product ${category.name}?`
      }).onOk(async () => {
        try {
    ...
    if (isAndroid) {
      visibleColumns.value = ['name', 'price', 'actions']
    }
